...
namespace MissionskyOA.Api.Common.Report
{
    public class ReportHelper
    {
        ...
        /// <param name="token">当前用户Token</param>
        /// <param name="reportFormat">报表格式</param>
        /// <param name="reprotParams">报表参数</param>
        /// <param name="isDetele">是否删除临时的报表文件</param>
        /// <returns>true or false</returns>
        public static bool SendReport(int reportId, string token, string reportFormat, Dictionary<string, string> reprotParams, bool isDetele = true)
        {
            //获取报表详细
            ReportModel report = ReportService.GetReportDetail(reportId);
            //报表配置
            NameValueCollection reportConfigs = ReportService.GetReportConfigs(reportId);
            
            if (report == null || reportConfigs == null || reportConfigs.Count < 1)
            {
                Log.Error("找不到报表。");
            ...
            //发送报表邮件
            SendEmail(report, token, reportPath);
        ...
        private static void SendEmail(ReportModel report, string token, string reportPath)
        {
            ...
            EmailClient.Send(new List<string> { user.Email }, null, report.Name, string.Format("生成报表{0}成功。请查看附件。", report.Name), attachments);
        }
    }
}